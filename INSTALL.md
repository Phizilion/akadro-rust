...
```toml
[dependencies]
akadro = { git = "ssh://git@<your-host>/<org>/akadro-rust.git", tag = "v0.1.1", \
           features = ["analytics", "data"] }
akadro-venue-mexc = { git = "ssh://git@<your-host>/<org>/akadro-rust.git", tag = "v0.1.1", \
                      features = ["net"] }
```

- **Always pin** with `tag = "v0.1.1"` (or `rev = "<sha>"`) for reproducible
...
- **Semver (important at 0.x):** while akadro is `0.y`, Cargo treats a **minor**
  ...
  precisely — `akadro = "=0.1.1"`, or a git `tag`/`rev` — and adopt new minors
